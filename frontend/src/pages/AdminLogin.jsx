...
const AdminLogin = () => {
  ...
  async function fetchTeamMembers() {
    try {
      ...
      // Safety Check: Ensure the data is actually an array before saving it
  ...
  const handleLogin = (e) => {
    ...
    // Safety Check: Only loop if userCredentials is an array
    ...
    const isTestLogin = email === "test@gmail.com" && password === "Hirotec@123";

    if (matchedUser || isTestLogin) {
  ...
  return (
    <div className="relative min-h-screen flex overflow-hidden bg-[#F5F0FF]">
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        {/* Pink Glow */}
        <div className="absolute -right-40 top-32 w-[650px] h-[650px] rounded-full bg-pink-300/40 blur-[180px]"></div>

        {/* Blue Glow */}
        <div className="absolute left-1/2 bottom-[-200px] -translate-x-1/2 w-[700px] h-[700px] rounded-full bg-indigo-300/20 blur-[200px]"></div>

        {/* Small Glow */}
        <div className="absolute top-10 left-1/3 w-[300px] h-[300px] rounded-full bg-purple-400/20 blur-[120px]"></div>
      </div>
